...
Pop_Rat_G_B <- function(Gpars, Lpars, DOSE, N) {
    ...
    idata <- 
        tibble(ID = 1:N) %>% 
        mutate( BW0     = rnormTrunc (N, min = 0.08, max = 0.32, mean = 0.2, sd = 0.06),
                KbileC  = rlnormTrunc (N, min = exp(-6.37), max = exp(-5.62), meanlog = -6.00, sdlog = 0.19),
                Free    = rlnormTrunc (N, min = exp(-5.87), max = exp(-5.36), meanlog = -5.62, sdlog = 0.13),
                PL      = rlnormTrunc (N, min = exp(0.73), max = exp(1.24), meanlog = 0.98, sdlog = 0.13),
                GFRC    = rnormTrunc  (N, min = 24.95, max = 57.13, mean = 41, sd = 8.208),
                PMilkM = rlnormTrunc (N, min = exp(0.22), max = exp(0.98), meanlog = 0.60, sdlog = 0.19),
                KMilk0 = rlnormTrunc (N, min = exp(-1.55), max = exp(-1.04), meanlog = -1.29, sdlog = 0.13), 
                Free_pup = rlnormTrunc (N, min = exp(-4.09), max = exp(-3.58), meanlog = -3.84, sdlog = 0.13),
                PL_pup  = rlnormTrunc (N, min = exp(0.66), max = exp(1.17), meanlog = 0.92, sdlog = 0.13),
                PRest_pup = rlnormTrunc (N, min = exp(-1.79), max = exp(-1.28), meanlog = -1.53, sdlog = 0.13),
...
PlotDat_Plas_L <- PlotDat_Plas_L %>% filter (Conc < 100 & Conc >0)
